...
- Nav hrefs in `src/content/home.ts` are absolute (`/#about`, not `#about`) so the shared header and footer resolve from inner pages; `What We Do`, `Partners`, `News & Articles`, and `Contact Us` already point at `/what-we-do`, `/partners`, `/news`, and `/contact`. When About lands, update its nav entry to the real route and `aboutLink` in `src/content/what-we-do.ts`, since the inner pages link out through it (`contactLink` there is already `/contact`, and `src/content/contact.ts` reads its route from it).

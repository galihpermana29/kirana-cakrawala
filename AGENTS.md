...
- `PillarHero` and `PillarCta` (in `src/components/what-we-do/`) are the shared inner-page hero and closing band - reuse them on new pages rather than writing another.
...
- Nav hrefs in `src/content/home.ts` are absolute (`/#about`, not `#about`) so the shared header and footer resolve from inner pages; `What We Do` and `News & Articles` already point at `/what-we-do` and `/news`. When another inner page lands, update its nav entry to the real route - and `aboutLink` / `contactLink` in `src/content/what-we-do.ts` if it is About or Contact, since the pillar pages link out through those.

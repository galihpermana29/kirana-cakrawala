...
- Nav hrefs in `src/content/home.ts` are absolute (`/#about`, not `#about`) so the shared header and footer resolve from inner pages; `What We Do`, `Partners`, and `News & Articles` already point at `/what-we-do`, `/partners`, and `/news`. When another inner page lands, update its nav entry to the real route - and `aboutLink` / `contactLink` in `src/content/what-we-do.ts` if it is About or Contact, since the pillar pages link out through those.
